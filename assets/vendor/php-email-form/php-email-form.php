...
class PHP_Email_Form {
    ...
    /*
     * Same as send but we use our own mailer
     */
    public function send2($form_type) {


        // Not currently used in Karkusha web-site, but may be implemented later
        if( $this->recaptcha_secret_key ) {
        ...
        if( $this->error ) {
            write_to_log([], 'error is '.$this->error);
            return $this->error;
        }

        require_once ('../app/inc/func.php');

        // If email was sent we can redirect to thank you page
        // The following zapier bot email will insert the lead into AlfaCRM and also send us an email with the lead's contact details

        // Determine which zapier inbox will handle this form submission. Each mailbox has a different template for parsing the info
        if($form_type == 'main_form')
            $zapier_mailbox = "moznb5yk@robot.zapier.com";

        if($form_type == 'newsletter')
            $zapier_mailbox = "3ps3i916@robot.zapier.com";


        if(send_email($zapier_mailbox, $subject, $message)) {
